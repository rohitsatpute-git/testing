...
import { createOrder, sendfullname } from "../actions/orderAction";
...
const PlaceOrder = ({ history, match }, props) => {
  const dispatch = useDispatch();
  const [sdkReady, setSdkReady] = useState(false);
  const userLogin = useSelector((state) => state.userLogin);
  const { userInfo } = userLogin;
  const orderCreate = useSelector((state) => state.orderCreate);
  const { order, success, error, loading: orderCreateLoading } = orderCreate;
  const cart = useSelector((state) => state.cart);
  const { shippingAddress, loading: shippingAdressSaveSuccessfully } = cart;
  ...
  useEffect(() => {
    ...
    if (success) {
      history.push(`/shipping/placeorder/order/${order._id}`);
    }
  ...
  async function displayRazorpay() {
    ...
    const result = await axios.post("/payment/orders", {
      amount: cart.totalPrice.toString() * 100,
    });
    ...
    const options = {
      ...
      handler: async function (paymentResults) {
        if (paymentResults.razorpay_order_id) {
          ...
          console.log("payOrderID", orderId);
  ...
  $(document).ready(function () {
    $("#downArrow").click(function () {
      $("div.mobileProductNotVisible").toggle();
    });
    $("#showProductList").click(function () {
      $(".checkout__product__list__container").toggle();
    });

    $("#editShippingInfo").click(function () {
      $(".checkout__shipping__info__saved").hide();
      $(".checkout__shipping__info").show();
    });

    $(".dropdown__on").click(function () {
      $(".dropdown__on").hide();
      $(".placeorder__dropdown__items").show();
      $(".dropdown__close").show();
    });

    $(".dropdown__close").click(function () {
      $(".dropdown__close").hide();
      $(".placeorder__dropdown__items").hide();
      $(".dropdown__on").show();
    });

    $("#editShippingInfo__desktop").click(function () {
      $(".placeorder__userDetail__form__sec__saved").addClass("hidden");
      $(".placeorder__userDetail__form__sec").removeClass("hidden");
    });
    ...
    $("form").on("submit", function (e) {
      ...
      e.preventDefault();
      $(".placeorder__userDetail__form__sec__saved").show();
      $(".placeorder__userDetail__form__sec").hide();
    });
  });

  const placeOrderBtnStatus = localStorage.getItem("shippingAddress")
    ? false
    : true;
  console.log(placeOrderBtnStatus);
  return (
    <>
      {!checkoutLoader ? (
      ...
      <div className="">
        <div className="placeorder__page bg-gray-50">
          {/* <CheckoutStep fullWidth step1 step2 /> */}

          <div className="col-lg-12 col-md-12 col-sm-12 col-12 d-flex flex-wrap">
            <div className=" hidden">
              <h2>Confirm your order products</h2>
              <div className="col-lg-12 col-md-12 col-sm-12 col-12  ">
                {cartItems.map((item, index) => (
                  <div className="col-lg-12 col-md-12 col-sm-12 col-12 d-flex flex-wrap align-items-center  placeorder__product__list">
                    <div className="col-lg-2 col-md-3 col-sm-3 col-3 placeorder__img__thumbnail ">
                      <img src={item.image[0]} width="100%" height="100%" />
                    </div>
                    <div className="col-lg-8 col-md-6 col-sm-6 col-6 myCart__product__div__title ">
                      <h6 className="z__block">{item.title}</h6>
                      <h6 className="z__block">
                        &#8377;{item.qty * item.price}
                      </h6>
                    </div>
                    <div className=" col-md-2 col-sm-2 col-2 formSelect ">
                      <h6>Qty : {item.qty}</h6>
                    </div>
                  </div>
                ))}
            ...
            <div className="p-5">
              <div className="">
                <div className="bg-white p-5 rounded-lg shadow-md border">
                  <h4 className="font-bold mb-2 text-lg">Checkout</h4>
                  <div className="placeorder__gap text-gray-500 font-semibold">
                    <h5>
                      <i className="fas fa-at mr-3"></i> {userInfo.email}
                    </h5>
                  </div>
                </div>

                <div className="bg-white p-5 rounded-lg shadow-md border mt-4 placeorder__userDetail__form__sec hidden">
                  <h4 className="font-bold text-lg capitalize mb-2">
                    Shipping information
                  </h4>
                  <div className="placeorder__userDetail__div__form">
                    <form>
                      <div class="mb-6">
                        <label
                          for="fullname"
                      ...
                      <div class="flex items-start mb-6">
                        ...
                        >
                          This address saved for fastest checkout next time.
                        </label>
                      </div>
                      <button
                        type="submit"
                    ...
                    >
                      <div className="form-floating mb-3 col-lg-6 col-sm-12 col-12 px-1">
                        <input
                          required="true"
                          title="Enter first and lastname"
                          pattern="^(\w\w+)\s(\w+)$"
                          type="text"
                      ...
                      <div className="form-floating mb-3 col-lg-12 col-sm-12 col-12">
                        <textarea
                          type="text"
                      ...
                      <div className="col-12 d-flex justify-content-between align-items-center">
                        <h4 className="placeorder__smallFont col-lg-8">
                          This address saved for fastest checkout next time .
                        </h4>
                ...
                <div className=" bg-gradient-to-br from-white/100 p-5 rounded-lg shadow-md border mt-4 placeorder__userDetail__form__sec__saved">
                  <div className="flex justify-between items-cente">
                    <h4 className="font-bold mb-3 text-lg">
                      Shipping information
                    </h4>
                    <div id="editShippingInfo__desktop">
                  ...
                  {localStorage.getItem("shippingAddress") != null ? (
                    <div className="px-5">
                      <h5 className="text-gray-500 font-semibold mb-2 ">
                        <i className="mr-2 fas fa-user"></i> {fullName}
                      </h5>
                      <h5 className="text-gray-500 font-semibold mb-2 ">
                        <i className="mr-2 fas fa-map-marker-alt"></i> {address}
                      </h5>
                      <h5 className="text-gray-500 font-semibold mb-2 ">
                    ...
                    <div className="p-5">
                      <h5 className="text-gray-500 font-semibold">
                        Please click on edit and fill the shipping form
                      </h5>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white p-5 rounded-lg shadow-md border mt-4">
                  <h4 className="font-bold mb-2 text-lg">Payment method</h4>
                  ...
                  <fieldset className="mt-4 px-5">
                    ...
                    <div class="flex items-center mb-4">
                      <input
                        ...
                        type="radio"
                        name="paymentMethod"
                        value="Credit card or Netbanking"
                        onChange={(e) => {
                          dispatch(savePaymentMethod(e.target.value));
                          setPaymentMethod(e.target.price);
                        }}
                      ...
                      <label
                        ...
                        class="block text-sm font-medium text-gray-900 dark:text-gray-300 capitalize"
                      >
                        credit card and netbanking
                      </label>
                    </div>
                    ...
                    <div class="flex items-center mb-4">
                      ...
                      <label
                        ...
                        class="block ml-2 text-sm font-medium text-gray-900 dark:text-gray-300 capitalize"
                      >
                        cash on Delivery
                      </label>
                    </div>
                  ...
                  {/* <div className="placeorder__gap">
                    ...
                    >
                      ...
                      <div className="checkoutpage__paymentMethod__radioBtn">
                        <input
                          type="radio"
                          name="paymentMode"
                          id="Credit card or Netbanking"
                          value="Credit card or Netbanking"
                          onChange={(e) => {
                            dispatch(savePaymentMethod(e.target.value));
                            setPaymentMethod(e.target.price);
                        ...
                        />
                         {" "}
                        <label for="Credit card or Netbanking">
                          Credit card or Netbanking
                        </label>
                        <br />
                      </div>
                ...
                <div className="bg-white p-5 rounded-lg shadow-md border mt-4">
                  <div className="flex justify-between">
                    ...
                    {dropdown ? (
                      <CloseIcon onClick={() => setDropdown(!dropdown)} />
                    ) : (
                  ...
                  {dropdown ? (
                    <div className="cart__seperate mt-4">
                      <div className=" max-h-screen overflow-scroll">
                        {cartItems.map((item, index) => (
                          <div className="flex gap-5 min-h-48 bg-white justify-evenly mb-1 py-4 px-2 border rounded items-center">
                            <div className="w-2/6 ">
                              <img
                  ...
                  {/* <div className="placeorder__dropdown__items">
                    {cartItems.map((item, index) => (
                      <div className="col-lg-12 col-md-12 col-sm-12 col-12 d-flex flex-wrap align-items-center myCart__product__div">
                        <div className=" cart__img__thumbnail">
                          <img src={item.image[0]} width="100%" height="100%" />
                        </div>
                        <div className="col-lg-6 col-md-6 col-sm-7 col-7 myCart__product__div__title">
                          <h6>{item.title}</h6>
                          <h6>&#8377;{item.qty * item.price}</h6>
                        </div>
                        <div className="col-lg-1 col-md-2 col-sm-2 col-2 formSelect">
                          <h6 align="center">{item.qty}</h6>
                        </div>
                      </div>
                    ))}
                  </div> */}
                ...
                <div className="flex justify-center items-center">
                  ...
                  <button
                    onClick={placeOrderHandler}
                    disabled={placeOrderBtnStatus}
                    className="btn col-7 col-sm-7 hidden"
                  >
